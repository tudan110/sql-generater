...
public class SQLGenerater {
	...
	private boolean printSettings(Scanner scanner) {
		...
		System.out.println("\n当前配置如下");
		System.out.println(StrUtil.format("数据库方言或模板组：{}", dialect));
	...
	private void askUpdateSettings(Scanner scanner) {
		...
		if (yesOrNo(scanner.next())) {
			...
			System.out.println(StrUtil.format("1. 数据库方言或模板组（{}）", getDialectSupported()));
			System.out.println("2. 待解析 Excel 路径");
			System.out.println(StrUtil.format("3. 模板类型（{}）", getAllTplName()));
			System.out.println("4. 输出模式（console, file）");
			if ("file".equals(mode)) {
				System.out.println("5. 输出路径");
			}

			switch (scanner.next()) {
				case "1":
					System.out.println(StrUtil.format("请输入数据库方言或模板组（{}}）", getDialectSupported()));
					setDialect(scanner.next());
					break;
				case "2":
	 ...
	 *
	 * @return String
	 * @date 2019-09-25 17:10:19
	 */
	private String getDialectSupported() {
	...
	private String getAllTplName() {
		return Arrays.stream(FileUtil.ls(StrUtil.format("{}{}/{}", ClassUtil.getClassPath(), "templates", dialect)))
				.map(File::getName).collect(Collectors.joining(", "));
	}
	...
	private void parseTableDDL(JSONObject table) {
		...
		switch (mode) {
			case "console":
				...
				System.out.println(FreemarkerUtil.parseFTL(StrUtil.format("{}/{}", dialect, ftlName), table));
				break;
			case "file":
				...
				writer.write(FreemarkerUtil.parseFTL(StrUtil.format("{}/{}", dialect, ftlName), table));
				break;
			default:
				break;
		}
